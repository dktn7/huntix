...
Huntix is a **modern arcane action anime** brought into a browser brawler. The reference feeling is:

- **Hunter x Hunter** — silhouette clarity, aura presence, elite hunter aesthetic
...
- Modern tailoring mixed with arcane battle armour — think gear that was designed, not found
...
```
1. Portrait / reference description
        ↓ GPT Image 1.5
...
All generated assets go into the main asset pipeline:
  ...
  textures/
    characters/
      dabik/
        ...
        frames/          ← extracted, keyed, normalised individual frames
      benzu/
      sereisa/
      vesol/
```

Experiment outputs and intermediate runs go into:
```
experiments/
  fal-image/
...
- `.agents/skills/fal-ai-image/SKILL.md` — queue workflow, model presets, cost tracking
- `.agents/skills/3d-model-optimization.md` — phase gate reference for when art enters the engine

---

## See Also

- `docs/VISUAL-DESIGN.md` — full art direction, aura colours, world tone, per-hunter appearance
...
- `bonus/huntix-character-art/prompts/` — ready-to-run prompt templates for all 4 hunters
